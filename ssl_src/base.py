...
class BaseSSLModule(pl.LightningModule):
    def __init__(self, base_encoder: str = "resnet50", lr: float = 0.2,
                 weight_decay: float = 1e-6, warmup_epochs: int = 10, **kwargs):
        super().__init__()
        ...
        def lr_lambda(epoch):
            if epoch < self.warmup_epochs:
                ...
                return 0.5 * (1 + torch.cos(torch.tensor(
                    ...
                    (self.trainer.max_epochs - self.warmup_epochs) * 3.14159
                )))
