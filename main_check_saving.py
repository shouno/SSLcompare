...
class ImprovedModelCheckpoint(ModelCheckpoint):
    """PyTorch Lightning標準機能 + 手動保存のハイブリッド"""
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        # 手動保存も実行（保険）
        if trainer.global_rank == 0:  # rank 0でのみ実行
            self._manual_save(trainer, pl_module)
    
    def _manual_save(self, trainer, pl_module):
        # 保存条件をチェック
        should_save = False
        ...
        # monitor チェック
        if self.monitor and self.monitor in trainer.callback_metrics:
        ...
        if should_save:
            filename = self._generate_filename(trainer)
            filepath = os.path.join(self.dirpath, filename)
            self._save_manual_checkpoint(trainer, pl_module, filepath)
    
    def _generate_filename(self, trainer):
        """ファイル名を生成"""
        metrics = trainer.callback_metrics
        
        # filenameパターンを解析してメトリクスを埋め込み
        if self.filename:
            try:
                # メトリクスの値を取得
                ...
                for key, value in metrics.items():
                    if isinstance(value, torch.Tensor):
                        format_dict[key] = value.item()
                    else:
                        format_dict[key] = value
                
                return self.filename.format(**format_dict)
            except (KeyError, ValueError):
                # フォーマットエラーの場合はシンプルな名前
                return f"epoch-{trainer.current_epoch:03d}.ckpt"
        else:
            return f"epoch-{trainer.current_epoch:03d}.ckpt"
    
    def _is_better_score(self, current_score):
        """スコアが改善したかチェック"""
    ...
    def _save_manual_checkpoint(self, trainer, pl_module, filepath):
        """手動でチェックポイントを保存"""
        try:
            checkpoint = {
                ...
                'lr_schedulers': [sch.state_dict() for sch in trainer.lr_schedulers],
                'hyper_parameters': pl_module.hparams,
                'callbacks': {},  # コールバック状態も保存可能
            }
            
            torch.save(checkpoint, filepath)
            print(f"✅ 手動保存成功: {filepath}")
            
            # ModelCheckpointの状態を更新
            if self.monitor and self.monitor in trainer.callback_metrics:
                self.best_model_score = trainer.callback_metrics[self.monitor]
...
class ForceModelCheckpoint(ModelCheckpoint):
    """強制的にチェックポイントを保存するコールバック"""
    
    def on_train_epoch_end(self, trainer, pl_module):
        print(f"\n🔧 ForceModelCheckpoint: エポック {trainer.current_epoch}")
        
        # 手動でファイルパスを生成
        filename = f"manual-epoch-{trainer.current_epoch:03d}.ckpt"
        filepath = os.path.join(self.dirpath, filename)
...
def cli_main():
    ...
    # 1. 無条件保存（verb_main.pyと同じパターン）
    #force_save = ForceModelCheckpoint(
    force_save = ModelCheckpoint(
